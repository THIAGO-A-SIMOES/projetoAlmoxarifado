...
public class Tela_Relatorios_Cliente extends javax.swing.JFrame {

    public Tela_Relatorios_Cliente() {
        initComponents();
    }
    ...
    private void preencherDadosNome() throws SQLException {
        try {
            ...
            String sql = "select * from relatorio_clientes where  Cliente  = '" + text_box.getText() + "' ";
            ...
            while (rs.next()) {
                ...
                String alojamentos = rs.getString("Alojamentos");
                String situacao = rs.getString("Situacao");

                DefaultTableModel model = (DefaultTableModel) TB_Cliente.getModel();
                model.addRow(new Object[]{sigla, cidade, codCidade, nome, cnpj, endereco, contato, alojamentos, situacao});
    ...
    private void preencherDadosCNPJ() {
       try {
            Connection conexao = ConexaoBanco.getConnection();
            String sql = "select * from relatorio_clientes where  CNPJ  = '" + text_box.getText() + "' ";
            PreparedStatement ps = conexao.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
    ...
    private void preencherDadosCodigo() {
        try {
            Connection conexao = ConexaoBanco.getConnection();
            String sql = "select * from relatorio_clientes where  CódCliente  = '" + text_box.getText() + "' ";
            PreparedStatement ps = conexao.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
    ...
    private void preencherDadosCodigoEstado() {
        try {
            Connection conexao = ConexaoBanco.getConnection();
            String sql = "select * from relatorio_clientes where  Estado  = '" + text_box.getText() + "' ";
            PreparedStatement ps = conexao.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
    ...
    private void preencherDadosCodigoCidade() {
        try {
            Connection conexao = ConexaoBanco.getConnection();
            String sql = "select * from relatorio_clientes where CNPJ = '"+ text_box.getText() +"' ";
            PreparedStatement ps = conexao.prepareStatement(sql);
    ...
    private void initComponents() {
        ...
        setTitle("Info de clientes vinculados a um alojamento");

        pesquisa.setBorder(javax.swing.BorderFactory.createTitledBorder("Relatorio de Cliente"));

        combo_box_geral.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Nome", "CNPJ", "Código do Cliente", "Código do Estado", "Código da Cidade" }));
        ...
        TB_Cliente.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Estado", "Cidade", "Código", "Nome", "CNPJ", "Endereço", "Contato", "Alojamentos", "Situação"
            }
        ) {
        ...
        if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
            ...
            if (combo_box_geral.getSelectedItem() == "Nome") {
                try {
                    ...
                    Logger.getLogger(Tela_Relatorios_Cliente.class.getName()).log(Level.SEVERE, null, ex);
                }
            ...
            } else if(combo_box_geral.getSelectedItem() == "Código do Estado"){
                preencherDadosCodigoEstado();
                if(TB_Cliente.getRowCount() == 0){
                    JOptionPane.showMessageDialog(this, "Não foram encontrados Clientes com esse Código de Estado!", "Confirmação", JOptionPane.ERROR_MESSAGE);
                }
            } else if (combo_box_geral.getSelectedItem() == "Código da Cidade"){
                preencherDadosCodigoCidade();
                if(TB_Cliente.getRowCount() == 0){
                    JOptionPane.showMessageDialog(this, "Não foram encontrados Clientes com esse Código de Cidade!", "Confirmação", JOptionPane.ERROR_MESSAGE);
                }
            }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(Tela_Relatorios_Cliente.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new Tela_Relatorios_Cliente().setVisible(true);
            }
        });
    }
